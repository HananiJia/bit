...
function getChannel(){
	$.ajax({
		url:'http://api.jirengu.com/fm/getChannels.php',//获取数据的网址接口
		dataType:'json',//后台和前端数据传输类型
		Method:'get',//数据传输方式为get或者post这里选择get
		success:function(response){//成功回调函数
			var channels=response.channels;//将从后台的值赋值到当前数据类型中
			var num=Math.floor(Math.random()*channels.length);
			var channelname=channels[num].name;
			var channelId=channels[num].channel_id;
			$('.record').text(channelname);//对html中class为record的类中添加text文本数据
...
function getmusic(){
	$.ajax({
		...
		success:function(ret){
			...
			var author=resource.artist;//获取歌手名称
			$('audio').attr('src',url);//对audio元素添加src属性的值$('.content').style()
			$('audio').attr('sid',sid);
			$('audio').attr('ssid',ssid);
			$('.musicname').text(title);//对class为musicname的元素添加文本
			$('.musicname').attr('title',title);//对class为music的元素添加tittle属性的值
			$('.musicer').text(author);
			$('.musicer').attr('author',author);
			$('.background').css({
...
function play(){
	myAudio.play();
	$('.maxcircle').removeClass('m-play').addClass('m-pause');
}
function pause()
{
	myAudio.pause();
	$('.maxcircle').removeClass('m-pause').addClass('m-play');
}
//播放暂停
$('.maxcircle').click(function(){
	if(myAudio.paused)
	{
		play()
	}
...
$(document).ready(getChannel());//页面在准备加载的时候调用getchannel函数获取数据